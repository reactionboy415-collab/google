...
# --- GLOBAL STATE ---
PROXY_STORAGE = {} # {url: {uses: 0, last_reset: datetime, status: "Online 🟢"}}
SCANNED_TOTAL = 0
...
def get_smart_proxy():
    # Lifecycle rule: Only use if uses < 4 and status is not a permanent error
...
async def cmd_stats(message: types.Message):
    ...
    total_proxies = len(PROXY_STORAGE)
    available_proxies = len([p for p, data in PROXY_STORAGE.items() if data['uses'] < 4])
    
    header = "*📊 INFINTY CORE SYSTEM REPORT*\n━━━━━━━━━━━━━━━━━━━━\n"
    header += f"📡 *Available Nodes:* `{available_proxies}` / `{total_proxies}`\n"
    header += f"🔍 *Pool Discovery:* `{SCANNED_TOTAL}`\n"
    header += f"🛠 *Engine:* `V15.5 lifecycle Optimized`\n\n"
    
    proxy_details = "*PROXY REGISTRY:*\n"
    for p, data in PROXY_STORAGE.items():
        clean_ip = p.replace('http://', '')
        usage_label = "Limit Reached 🟡" if data['uses'] >= 4 else data['status']
        proxy_details += f"🔹 `{clean_ip}` | Usage: `{data['uses']}/4` | `{usage_label}`\n"
    
    full_msg = header + proxy_details
    if len(full_msg) > 4000:
        for i in range(0, len(full_msg), 4000):
            await message.answer(full_msg[i:i+4000])
    else:
        await message.answer(full_msg)

@dp.message(F.text == "/start")
async def cmd_start(message: types.Message):
    await message.answer("*🔱 Welcome to NoteGPT Infinity v15.5 🔱*\n\nProfessional AI Music Generation - Optimized Lifecycle Edition.\n\n*Instructions:*\nJust send me a song topic or theme. I will generate professional lyrics and an MP3 track instantly.")
...
async def handle_music_generation(message: types.Message):
    ...
    status_msg = await message.answer("*🎯 Connecting to High-Speed Node...*")
    
    try:
        # STEP 1: Lyrics (Pollinations Engine)
        ...
        await status_msg.edit_text("*🎼 AI Studio is composing your track. Fixed Duration: 1 Min...*")
        ...
        if data.get("code") == 100000:
            ...
            await status_msg.edit_text("*⚠️ Studio Error:* Node failed to respond. Please retry.")

    except Exception:
        PROXY_STORAGE[proxy]['status'] = "Error 🔴"
        await status_msg.edit_text("*⚠️ Technical Error:* Connection timeout. Try again.")
    finally:
        gc.collect()
