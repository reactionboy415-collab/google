...
async def handle_music(message: types.Message):
    ...
    try:
        # --- 📝 LYRICS LOGIC (TRANSLITERATED) ---
        p = (
            f"Write professional song lyrics about '{topic}'. "
            f"STRICT INSTRUCTION: Use the target language of the prompt but write ONLY in English/Latin characters (Transliteration). "
            f"Structure: [Verse], [Chorus]. Plain text."
        )
        ly_url = f"https://text.pollinations.ai/{quote(p)}"
