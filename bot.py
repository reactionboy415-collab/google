...
class HybridEngine:
    ...
    async def start_via_tunnel(self, payload, cookies):
        """Uses Vercel to get a fresh IP for generation."""
    ...
    async def poll_locally(self, cid, cookies):
        """Polls status directly for maximum speed (matches your local script)."""
...
async def handle_music(message: types.Message):
    ...
    try:
        # 1. LYRICS
        ly_url = f"https://text.pollinations.ai/{quote(f'Write song lyrics about {topic}. Structure: [Verse], [Chorus].')}"
        lyrics = (await asyncio.to_thread(requests.get, ly_url)).text
        await message.answer(f"📜 **Lyrics:**\n\n{lyrics[:3000]}")

        # 2. START GENERATION (VIA VERCEL TUNNEL)
