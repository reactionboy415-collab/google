...
async def check_proxy_task(p_url):
    global PASSED_THIS_CYCLE
    try:
        scraper = cloudscraper.create_scraper()
        # Fast 4s check to ensure only superfast proxies are kept
...
async def proxy_engine():
    global WORKING_PROXIES, SCANNED_POOL_SIZE, TOTAL_TESTED_THIS_CYCLE, PASSED_THIS_CYCLE
    while True:
        temp_pool = set()
        # 1. Fetch from Supabase + 5 Premium Sources
        sources = [
        ...
        if temp_pool:
            # Test 600 proxies in parallel chunks to avoid memory crash
        ...
        gc.collect()
        await asyncio.sleep(120) # Faster refresh: 2 mins
...
async def cmd_start(message: types.Message):
    await message.answer("🎼 **NOTEGPT BEAST v18.0**\n\nBhai, engine hyper-speed par hai. Stats check karein ya music generate karein!", reply_markup=main_kb)
...
async def cmd_stats(message: types.Message):
    ...
    msg = (
        ...
        f"🔍 Total Pool Size: `{SCANNED_POOL_SIZE}`\n"
        f"🧪 Last Cycle Tested: `{TOTAL_TESTED_THIS_CYCLE}`\n"
        f"✅ Working Proxies: `{len(WORKING_PROXIES)}` 🔥\n"
        f"⚡ **Bypass Speed:** Superfast"
    )
...
async def handle_ai(message: types.Message):
    if message.text in ["🎵 Create Music", "📊 My Stats", "⚙️ Help"]:
        if message.text == "🎵 Create Music": await message.answer("Topic bhejein bhai!")
        return

    if not WORKING_PROXIES:
        await message.answer(f"⏳ **Warming up Engine...**\nCurrently testing `{TOTAL_TESTED_THIS_CYCLE}` proxies. 10s ruko.")
        return

    msg = await message.answer(f"🚀 **Bypassing NoteGPT Cloud...**\n{get_progress_bar(10)}")
    
    # Auto-Retry Logic: Try up to 3 different proxies if one fails
    for attempt in range(3):
        ...
        try:
            payload = {"prompt": message.text, "lyrics": "", "duration": 0}
            res = await asyncio.to_thread(scraper.post, "https://notegpt.io/api/v2/music/generate", json=payload, timeout=20)
            data = res.json()
            ...
            if data.get("code") == 100000:
                ...
                for i in range(2, 12):
                    ...
                    if s_data.get("status") == "success":
                        ...
                        await message.answer_audio(BufferedInputFile(audio.content, "song.mp3"), caption=f"✅ Done! (Attempt {attempt+1})")
                        await msg.delete(); return
                break # Exit retry loop if success
            else: continue # Try next proxy if code is not 100000
        except: continue
    
    await msg.edit_text("❌ All bypass attempts failed. Prompt thoda change karke dekho bhai!")
