...
# --- GLOBAL STATE ---
PROXY_STORAGE = {} # Keys: proxy_url, Values: {uses: int, last_reset: datetime, status: str}
WORKING_PROXIES = []
SCANNED_TOTAL = 0
...
bot = Bot(token=API_TOKEN, default_bot_properties={"parse_mode": ParseMode.MARKDOWN})
dp = Dispatcher()
...
async def handle_music_generation(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if data.get("code") == 100000:
            ...
            await status_msg.edit_text("⚠️ **Studio Error:** The selected node failed to respond. Retrying with a new proxy...")

    except Exception as e:
        if proxy in WORKING_PROXIES:
            PROXY_STORAGE[proxy]['status'] = "Error 🔴"
            WORKING_PROXIES.remove(proxy)
        await status_msg.edit_text(f"⚠️ **Technical Error:** Connection timed out. Please try again.")
    finally:
        gc.collect()
        await state.clear()
