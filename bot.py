...
class HybridEngine:
    ...
    async def start_via_tunnel(self, payload, cookies):
        ...
        return await asyncio.to_thread(requests.post, target, json=data, timeout=30)
...
async def cmd_start(message: types.Message):
    welcome_text = (
        ...
        "🎼 How to use:\n"
        "Simply send me a topic, a mood, or a detailed prompt (e.g., 'Emotional rap about life in Mumbai').\n\n"
...
async def handle_music(message: types.Message):
    ...
    try:
        # --- 📝 LYRICS LOGIC (TRANSLITERATED & REASONING FILTER) ---
        p = (
            f"Write professional song lyrics about '{topic}'. "
            "STRICT INSTRUCTION: Use the target language of the prompt but write ONLY in English/Latin characters (Transliteration). "
            "Structure: [Verse], [Chorus]. Plain text only. Do not include reasoning, assistant thoughts, or JSON."
        )
        # Using a model parameter to help force plain text output
        ...
        # Filter out reasoning/JSON blocks if they appear
        if "reasoning_content" in raw_output or "role" in raw_output:
            # Splitting to find the actual content if it returns a JSON-like string
            lyrics = raw_output.split("\n")[-1].replace('}', '').replace('"', '').strip()
            if not lyrics or len(lyrics) < 20:
                lyrics = "Lyrics generation was noisy. Please try again."
        else:
            lyrics = raw_output
        
        await message.answer(f"📜 Lyrics:\n\n{lyrics[:3500]}")

        await safe_edit(status_msg, "🚀 Bypassing limits & starting studio...")
