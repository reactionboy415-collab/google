...
# --- 🔑 CREDENTIALS ---
API_TOKEN = '8540275734:AAE0jN4aRgVhVzf1bBIB0y_LyxvVjU4aFjo'
ADMIN_ID = 7840042951
...
async def ask_duration(message: types.Message, state: FSMContext):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="1 - 2 Minute", callback_data="dur_0")],
        [InlineKeyboardButton(text="2 - 3 Minute", callback_data="dur_0")]
    ])
...
async def handle_music_generation(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not proxy:
        await callback.message.edit_text("*⌛ System Busy:* Daily proxy limit (4/4) reached. Resetting in 24h.")
        return
    status_msg = await callback.message.edit_text("*🎯 Connecting to High-Speed Node...*")
    try:
        # STEP 1: Lyrics (Updated to Pollinations Model)
        ...
        if data.get("code") == 100000:
            ...
            await status_msg.edit_text("*⚠️ Studio Error:* Node failed. Retrying...")
    except Exception:
        if proxy in WORKING_PROXIES:
