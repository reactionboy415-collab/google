...
WORKING_PROXIES = []
SCANNED_TOTAL = 0
PROXY_USAGE = {} # Stores {proxy_url: [timestamp1, timestamp2...]}
...
def clean_old_usage():
    """Removes timestamps older than 24 hours from PROXY_USAGE"""
    now = time.time()
...
def get_available_proxy():
    """Returns a proxy that has been used less than 4 times in the last 24h"""
    clean_old_usage()
    available = [p for p in WORKING_PROXIES if len(PROXY_USAGE.get(p, [])) < 4]
    if not available:
        return None
    selected = random.choice(available)
    if selected not in PROXY_USAGE:
        PROXY_USAGE[selected] = []
...
async def handle_music(message: types.Message):
    ...
    if not WORKING_PROXIES or not proxy:
        await message.answer("⌛ **System Resource Limit.** All active proxies are currently on cooldown or being validated. Please retry in a few minutes.")
        return
    ...
    try:
        ...
        # --- LYRICS GENERATION (POLLINATIONS AI via PROXY) ---
        ...
        # Use scraper with proxy to avoid rate limits on server IP
        lyric_res = await asyncio.to_thread(scraper.get, url, timeout=30)
